...
contract NiftyStaticCalls {
    ...
    function returnTxMessageToSign(bytes memory txData,
                                address des_add,
                                uint value) public view returns(bytes32) {
        NiftyWalletContract NW_instance = NiftyWalletContract(msg.sender);
        uint NiftyWalletTxCount = NW_instance.returnWalletTxCount();
        mStruct memory message = mStruct(msg.sender, des_add, value, NiftyWalletTxCount, txData);
